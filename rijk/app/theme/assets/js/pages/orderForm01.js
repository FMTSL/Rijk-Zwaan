...
$("#id_customer").on("change", function () {
  ...
  $.get(`/order-to-order/client/${id}`, function (dd) {
    ...
    if (parseInt($("#salanova option:selected").val()) === 1) {
      let valor2 = parseFloat(
        $("#id_salanova option:selected").text().replace("%", "")
      );

      $("#category_basic_discount, #desconto, #discount_form").val(valor2);
    } else {
    ...
    $(`#additional_discount, #payment_type, #salanova`).prop("disabled", false);
  });
});
