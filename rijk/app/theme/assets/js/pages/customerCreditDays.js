...
function update(id) {
  $.get(`/billing-term/${id}`, function (dd) {
    console.log(dd);
    $("#new").modal("show");
    $("#form #days").val(dd.days);
    $("#form #ticket").val(dd.ticket);
    $("#form #id").val(dd.id);
    $("#form h4").text("Update");
    $("#form p").text(`Update ${dd.name}`);
    $("#form button").text("Update");
    $("#form").attr("action", "/billing-term/update");
  });
}

function deletar(id) {
  $.ajax({
    url: `/billing-term/delete`,
    type: "delete",
    data: { id },
    success: function (dd) {
      console.log(dd);
...
$.get(`/billing-term/lists/all`, function (dd) {
