...
function update(id) {
  $.get(`/product-crop/${id}`, function (dd) {
    console.log(dd);
    $("#new").modal("show");
    $("#form #input_name").val(dd.name);
    $("#form #id").val(dd.id);
    $("#form h4").text("Update Product Crop");
    $("#form p").text(`Update Product Crop ${dd.name}`);
    $("#form button").text("Update");
    $("#form").attr("action", "/product-crop/update");
  });
}

function deletar(id) {
  $.ajax({
    url: `/product-crop/delete`,
    type: "delete",
    data: { id },
    success: function (dd) {
      console.log(dd);
...
$.get(`/products-crop/lists/all`, function (dd) {
