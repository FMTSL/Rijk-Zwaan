...
function update(id) {
  $.get(`/customer-type/${id}`, function (dd) {
    console.log(dd);
    $("#new").modal("show");
    $("#form #input_name").val(dd.name);
    $("#form #slug").val(dd.slug);
    $("#form #id").val(dd.id);
    $("#form h4").text("Update Customer type");
    $("#form p").text(`Update customer type ${dd.name}`);
    $("#form button").text("Update");
    $("#form").attr("action", "/customer-type/update");
  });
}
...
$.get(`/customer-type/lists/all`, function (dd) {
