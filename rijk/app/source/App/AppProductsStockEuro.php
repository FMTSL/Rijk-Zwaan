...
class AppProductsStockEuro
{
    ...
    {
        $itens = $this->acoes->getByFieldAllTwo('productsStockEuro', 'id_variety', $data['id'], 'status', 'true');
        if ($itens) {
    ...
    public function listViewAll($data): void
{
    ...
    {
        $itens = $this->acoes->getByField('productsStockEuro', 'id', $data['id']);
        header('Content-Type: application/json');
        $json = json_encode([
    ...
    {
        $item = (new ProductsStockEuro())->findById($id);
