        ...
        if (!empty($files["csv"])) {
            ...
            try {
                ...
                foreach ($products as $key => $prods) {
                    $productsCount = $prods['ID'] ? $this->acoes->counts('productsStock', 'id', $prods['ID']) : 0;
                    //dd($prods['Sales Variety']);
                    if ($productsCount == 0) {
                        $varietyFind = $this->acoes->getByField('productsVariety', 'name', $prods['Sales Variety']);
                        $packagingFind = $this->acoes->getByField('productsPackaging', 'name', $prods['Packaging Size and Unit']);

                        $prod = new ProductsStock();
                        $prod->id_package = $packagingFind->id;
                        $prod->id_variety = $varietyFind->id;
                        $prod->value = $prods['Price'];
                        $prod->weight = $prods['Weight'];
                        $prod->quantity = $prods['Quantity'];
                        $prod->article_number = $prods['Article Number'];
                        $prod->sub_article_number = $prods['Sub Article Number'];
                        $prod->status = true;
                        $prod->save();
                    } else {
                        ...
                        $prod = (new ProductsStock())->findById($prods['ID']);
                        $prod->id_package = $packagingFind->id;
                        $prod->id_variety = $varietyFind->id;
                        $prods['Price'] ? $prod->value = $prods['Price'] : '';
                        $prods['Weight'] ? $prod->weight = $prods['Weight'] : '';
                        $prods['Quantity'] ? $prod->quantity = $prods['Quantity'] : '';
                        $prod->article_number = $prods['Article Number'];
                        $prod->status = $prods['Status'] == "S" ? "TRUE" : "FALSE";
                        $prod->save();
                    }
                }
