...
function update(id) {
  $.get(`/user/${id}`, function (dd) {
    console.log(dd);
    $("#new").modal("show");
    $("#form #input_name").val(dd.name);
    $("#form #email").val(dd.email);
    $("#form #phone").val(dd.phone);
    $(`#form #roles option[value=${dd.roles}]`).prop("selected", true);
    $("#form #id").val(dd.id);
    $("#form h4").text("Update User");
    $("#form p").text(`Update User ${dd.name}`);
...
$.get(`/users/lists/all`, function (dd) {
